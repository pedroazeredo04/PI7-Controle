...
extern void pid_setProportionalGain ( long gain);
extern void pid_setIntegralGain ( long gain);
extern void pid_setDerivativeGain ( long gain);
extern void pid_setPoint ( long position);
...
extern void mot_setExcitation ( int excitation);
extern void mot_setExcitationRobot(int excitation);
# 9 "pid.c" 2

# 1 "./position_sensor.h" 1
# 11 "./position_sensor.h"
void pos_init(void);
# 20 "./position_sensor.h"
void pos_UpdatePosition(char port, char init, char dir);

void pos_setCurrentPosition(long pos);

long pos_getCurrentPosition(void);
# 10 "pid.c" 2


static int excitation;

static long currentPosition;
static long setPoint;
static long integralErrorScaled;
static long integralError;
static long previousError;
static long kProportional;
static long kIntegral;
static long kDerivative;

int pid_scaleExcitation ( long activation) {

   excitation = ( int ) activation;
   if (excitation > 0)
...
static long activation;
static long error;
static int excitation;

static long half_PID_PERIOD = 3;
static long PID_PERIOD = 6;
...
void pid_pid(void) {
  ...
  integralError += (kIntegral * (half_PID_PERIOD)) * (error + previousError);
  integralErrorScaled = integralError >> 10;
...
void pid_setPoint(long position) {
   setPoint = position;
}

void pid_setDerivativeGain(long gain) {
  kDerivative = gain;
}

void pid_setIntegralGain(long gain) {
  kIntegral = gain;
}

void pid_setProportionalGain(long gain) {
  kProportional = gain;
}
