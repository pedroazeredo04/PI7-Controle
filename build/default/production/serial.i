...
# 10 "C:\\Program Files\\Microchip\\xc8\\v2.41\\pic\\include\\c90\\conio.h" 2 3
...
# 88 "C:\\Program Files\\Microchip\\xc8\\v2.41\\pic\\include\\c90\\stdio.h" 3
...
void putch(char c)
{
 while(!TXIF)
 {
  if (OERR) { TXEN=0; TXEN=1; CREN=0; CREN=1; } if (FERR) { dummy=RCREG; TXEN=0; TXEN=1; };
  __asm("clrwdt");
 }
 TXREG=c;
...
unsigned char getch_timeout(void)
{
 ...
 for (i=2;i!=0;i--)
 {
  timeout_int=(long)(((240000)/16)*(20000000/1000000/4));

  while ((unsigned char)(timeout_int>>8)!=0)
  {
   __asm("clrwdt");
   timeout_int--;
   if (RCIF)
...
void putinthex(unsigned int c)
{
 ...
 putchhex((*(((unsigned char *)&c)+1)));
 putchhex((*(((unsigned char *)&c)+0)));
